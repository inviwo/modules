...
struct Wrapper0 {
    bool set(vtkStreaklineFilter& filter) {
...
struct Wrapper6 {
    bool set(vtkStreaklineFilter& filter) {
        filter.SetDisableResetCache(property.get());
        return true;
    }
    BoolProperty property{"DisableResetCache", "DisableResetCache",
                          R"(Prevents cache from getting reset so that new computation
always start from previous results.)"_help,
                          false};
};
...
template <>
struct VTKTraits<vtkStreaklineFilter> {
    static constexpr std::string_view uri = "vtk.vtkStreaklineFilter";
    static constexpr std::string_view className = "vtkStreaklineFilter";
    static constexpr std::string_view identifier = "StreakLine";
    static constexpr std::string_view displayName = "StreakLine";
...
void registervtkStreaklineFilter(InviwoModule* module) {
    module->registerProcessor<VTKGenericProcessor<vtkStreaklineFilter>>();
}
