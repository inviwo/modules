...
class NetCDFVolumeSequenceSource(genericnetcdfsource.GenericNetCDFSource):
    def __init__(self, id, name):
        genericnetcdfsource.GenericNetCDFSource.__init__(
            self, id, name, outputDimension=4)
        ...
        self.dataRange.readonlyDependsOn(
            self.overwriteDataRange, lambda x: not x.value)

        self.overwriteModel = BoolProperty(
            "overwriteModel", "Overwrite Model Matrix", False)
        ...
        self.modelMatrix.readonlyDependsOn(
            self.overwriteModel, lambda x: not x.value)
