...
static void nvg__segmentDir(NVGpickScene* ps, NVGpickSubPath* psp, NVGsegment* seg, float t,
                            float d[2]);
static void nvg__bezierBounds(const float* points, float* bounds);
static void nvg__bezierInflections(const float* points, int coord, int* ninflections,
                                   float* inflections);
...
static int nvg__pickSceneAddPoints(NVGpickScene* ps, const float* xy, int n) {
    ...
    if (ps->npoints + n > ps->cpoints) {
        ...
        float* points = static_cast<float*>(realloc(ps->points, sizeof(float) * 2 * cpoints));
        if (points == NULL) return -1;
        ps->points = points;
        ps->cpoints = cpoints;
    }
    i = ps->npoints;
    if (xy != NULL) memcpy(&ps->points[i * 2], xy, sizeof(float) * 2 * n);
    ps->npoints += n;
    return i;
}

static void nvg__pickSubPathAddSegment(NVGpickScene* ps, NVGpickSubPath* psp, int firstPoint,
...
static void nvg__segmentDir(NVGpickScene* ps, NVGpickSubPath* psp, NVGsegment* seg, float t,
    ...
    switch (seg->type) {
        ...
        case NVG_BEZIERTO:
            ...
            d[0] = 3.0f * omt2 * (x1 - x0) + 6.0f * omt * t * (x2 - x1) + 3.0f * t2 * (x3 - x2);
            d[1] = 3.0f * omt2 * (y1 - y0) + 6.0f * omt * t * (y2 - y1) + 3.0f * t2 * (y3 - y2);
...
static void nvg__pickSubPathAddStrokeSupports(NVGpickScene* ps, NVGpickSubPath* psp,
                                              float strokeWidth, int lineCap, int lineJoin,
                                              float miterLimit) {
    ...
    for (s = 0; s < nsegments; ++s) {
        ...
        if (seg->flags & NVG_PICK_CORNER && prevseg != NULL) {
            ...
            if (lineJoin == NVG_MITER || lineJoin == NVG_BEVEL) {
                ...
                // miterLimit is hit.
                if (lineJoin == NVG_BEVEL || ((M2 * miterLimit * miterLimit) < 1.0f)) {
                    ...
                    supportingPoints[ns++] =
                        points[firstPoint + 1] + seg->miterDir[1] * strokeWidth;
                }
            } else if (lineJoin == NVG_ROUND) {
                // ... and at the midpoint of the corner arc

                float vertexN[2] = {-seg->startDir[0] + prevseg->endDir[0],
        ...
        if (seg->flags & NVG_PICK_CAP) {
            switch (lineCap) {
                ...
                case NVG_ROUND:
                    ...
                    supportingPoints[ns++] =
                        points[firstPoint + 1] - seg->startDir[1] * strokeWidth;
                    break;
            }
        }
        ...
        if (seg->flags & NVG_PICK_ENDCAP) {
            ...
            switch (lineCap) {
                ...
                case NVG_SQUARE:
                    ...
                    // out along the direction
                    supportingPoints[ns++] =
                        supportingPoints[end + 0] + seg->endDir[0] * strokeWidth;
                    supportingPoints[ns++] =
                        supportingPoints[end + 1] + seg->endDir[1] * strokeWidth;
                    supportingPoints[ns++] =
                        supportingPoints[end + 2] + seg->endDir[0] * strokeWidth;
                    supportingPoints[ns++] =
                        supportingPoints[end + 3] + seg->endDir[1] * strokeWidth;
                    break;
                case NVG_ROUND:
                    // Add one additional support for the round cap along the dir
...
static NVGpickPath* nvg__pickPathCreate(NVGcontext* context, int id, int forStroke) {
        ...
        switch (cmd) {
            ...
            case NVG_BEZIERTO:
                ...
                if (ninflections) {
                    ...
                    for (infl = 0; infl < ninflections; ++infl) {
                        if (nvg__absf(inflections[infl] - previnfl) < NVG_PICK_EPS) continue;
                        ...
                        nvg__pickSubPathAddSegment(ps, psp, firstPoint - 1, cmd,
                                                   (infl == 0) ? NVG_PICK_CORNER : 0);
            ...
            case NVG_CLOSE:
                if (ps->points[(ps->npoints - 1) * 2] != start[0] ||
                    ...
                    nvg__pickSubPathAddSegment(ps, psp, firstPoint - 1, NVG_LINETO,
                                               NVG_PICK_CORNER);
                }
                psp->closed = 1;
            ...
            case NVG_WINDING:
                ...
                if (psp->winding == NVG_HOLE) hasHoles = 1;
                i += 2;
                break;
            default:
                i++;
    ...
    for (curpsp = psp; curpsp; curpsp = curpsp->next) {
        if (forStroke)
            nvg__pickSubPathAddStrokeSupports(ps, curpsp, pp->strokeWidth, pp->lineCap,
                                              pp->lineJoin, pp->miterLimit);
        else
...
static void nvg__freePickPath(NVGpickScene* ps, NVGpickPath* pp) {
    ...
    ps->freePaths = pp;
    if (pp->next == NULL) ps->lastPath = pp;
}
...
void nvg__deletePickScene(NVGpickScene* ps) {
    ...
    if (ps->picked) free(ps->picked);
    if (ps->points) free(ps->points);
    if (ps->segments) free(ps->segments);

    free(ps);
}

static NVGpickScene* nvg__pickSceneGet(NVGcontext* ctx) {
    if (ctx->pickScene == NULL) ctx->pickScene = nvg__allocPickScene();
...
static void nvg__bezierEval(const float* points, float t, float* tpoint) {
    ...
    tpoint[0] = points[0] * omt3 + points[2] * 3.0f * omt2 * t + points[4] * 3.0f * omt * t2 +
                points[6] * t3;

    tpoint[1] = points[1] * omt3 + points[3] * 3.0f * omt2 * t + points[5] * 3.0f * omt * t2 +
                points[7] * t3;
}
...
static void nvg__bezierInflections(const float* points, int coord, int* ninflections,
                                   float* inflections) {
    float v0 = points[0 * 2 + coord], v1 = points[1 * 2 + coord], v2 = points[2 * 2 + coord],
          v3 = points[3 * 2 + coord];
    float t[2];
    float a, b, c, d;
...
static int nvg__intersectBezier(const float* points, float x, float y) {
    float x0 = points[0 * 2 + 0], x1 = points[1 * 2 + 0], x2 = points[2 * 2 + 0],
          x3 = points[3 * 2 + 0];
    float y0 = points[0 * 2 + 1], y1 = points[1 * 2 + 1], y2 = points[2 * 2 + 1],
          y3 = points[3 * 2 + 1];
    ...
    for (iter = 0; iter < 6; ++iter) {
        ...
        ty = y0 * omt3 + y1 * 3.0f * omt2 * t + y2 * 3.0f * omt * t2 + y3 * t3;

        // Newton iteration
        dty = 3.0f * omt2 * (y1 - y0) + 6.0f * omt * t * (y2 - y1) + 3.0f * t2 * (y3 - y2);
    ...
    tx = x0 * omt3 + x1 * 3.0f * omt2 * t + x2 * 3.0f * omt * t2 + x3 * t3;
...
static void nvg__closestBezier(const float* points, float x, float y, float* closest, float* ot) {
    ...
    for (iter = 0; iter < 6; ++iter) {
        ...
        ddty = 6.0f * omt * (y2 - 2.0f * y1 + y0) + 6.0f * t * (y3 - 2.0f * y2 + y1);

        dtx = 3.0f * omt2 * (x1 - x0) + 6.0f * omt * t * (x2 - x1) + 3.0f * t2 * (x3 - x2);

        ddtx = 6.0f * omt * (x2 - 2.0f * x1 + x0) + 6.0f * t * (x3 - 2.0f * x2 + x1);
...
static int nvg__pickSubPathStroke(const NVGpickScene* ps, const NVGpickSubPath* psp, float x,
                                  float y, float strokeWidth, int lineCap, int lineJoin) {
    int nsegments;
        ...
        if (nvg__pointInBounds(x, y, seg->bounds) != 0) {
            ...
            if ((t >= NVG_PICK_EPS && t <= (1.0f - NVG_PICK_EPS)) ||
                ...
                if (!(seg1->flags & NVG_PICK_BEVEL)) {
                    ...
                    if (nvg__absf(prevNDist) < strokeWidth && nvg__absf(curNDist) < strokeWidth) {
                        return 1;
                    }
                } else {
                    ...
                    if ((seg1->miterDir[0] * d[0] + seg1->miterDir[1] * d[1]) < 0.0f) return 1;
                }
            }
        }
    }
...
static int nvg__pickSubPath(const NVGpickScene* ps, const NVGpickSubPath* psp, float x, float y) {
    ...
    for (s = 0; s < nsegments; ++s, ++seg) {
        if ((seg->bounds[1] - NVG_PICK_EPS) < y && (seg->bounds[3] - NVG_PICK_EPS) > y &&
            seg->bounds[2] > x) {
            // Line hits the box.
            switch (seg->type) {
                case NVG_LINETO:
                    if (seg->bounds[0] > x)
                        ...
                        nintersections +=
                            nvg__intersectLine(&ps->points[seg->firstPoint * 2], x, y);
                    break;
                ...
                case NVG_BEZIERTO:
                    if (seg->bounds[0] > x)
                        ...
                        // Line originates inside the box.
                        nintersections +=
                            nvg__intersectBezier(&ps->points[seg->firstPoint * 2], x, y);
                    break;
...
static int nvg__pickPathTestBounds(const NVGpickScene* ps, const NVGpickPath* pp, float x,
                                   float y) {
...
int nvgHitTestAll(NVGcontext* ctx, float x, float y, int flags, int* ids, int maxids) {
    ...
    if (ctx->pickScene == NULL) return 0;
            ...
            if (nvg__pickPathTestBounds(ps, pp, x, y)) {
                ...
                if (hit) {
                    if (npicked == ps->cpicked) {
                        ...
                        NVGpickPath** picked = static_cast<NVGpickPath**>(
                            realloc(ps->picked, sizeof(NVGpickPath*) * ps->cpicked));
    ...
    for (id = 0; id < maxids; ++id) ids[id] = ps->picked[id]->id;
...
int nvgHitTest(NVGcontext* ctx, float x, float y, int flags) {
    ...
    if (ctx->pickScene == NULL) return -1;
...
int nvgInFill(NVGcontext* ctx, float x, float y) {
    ...
    if (nvg__pointInBounds(x, y, pp->bounds) == 0) return 0;
...
static void nvg__pickSceneInsert(NVGpickScene* ps, NVGpickPath* pp) {
    ...
    // single cell)
    level = nvg__mini(base - nvg__countBitsUsed(cellbounds[0]),
    ...
    *cell = pp;
    ...
    if (ps->paths == NULL) ps->lastPath = pp;
    pp->next = ps->paths;
    ps->paths = pp;
...
void nvg__pickBeginFrame(NVGcontext* ctx, int width, int height) {
    ...
    if (ps->picked == NULL) {
        ps->cpicked = 16;
        ps->picked = static_cast<NVGpickPath**>(malloc(sizeof(NVGpickPath*) * ps->cpicked));
    }
