...
NANOVG_API NVGcontext* nvgCreateGL3(int flags);
NANOVG_API void nvgDeleteGL3(NVGcontext* ctx);

NANOVG_API int nvglCreateImageFromHandleGL3(NVGcontext* ctx, GLuint textureId, int w, int h,
                                            int flags);
NANOVG_API GLuint nvglImageHandleGL3(NVGcontext* ctx, int image);
...
static int glnvg__renderCreate(void* uptr) {
    ...
    static const char* fillVertShader =
        ...
        "	ftcoord = tcoord;\n"
        "	fpos = vertex;\n"
        "	gl_Position = vec4(2.0*vertex.x/viewSize.x - 1.0, 1.0 - 2.0*vertex.y/viewSize.y, "
        "0, 1);\n"
        "}\n";
